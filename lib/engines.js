...
export async function runAllEngines({
  ...
  compat1,
  compat2,
  compatScore   // NEW: passed in so Shopify summary can show same value
}) {
  ...
  // Person 1 base reading (used as foundation for compatibility)
  const palmistry = await analyzePalm(uploadedFile);
  const numerology = await analyzeNumerology(question);
  const astrology = await analyzeAstrology(question);
  ...
  return {
    ...
    triad,
    compat: compatOut,
    compatScore       // ensures Shopify front-end can read score
  };
}
